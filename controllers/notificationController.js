...
const { Notification, News } = require('../models');
...
const broadcastNotification = async (req, res) => {
  try {
    ...
    const result = await sendBroadcastNotification(normalized.payload);
    ...
    return res.status(201).json({
      success: true,
      message: `Notification sent to ${result.sentCount} user(s)`,
